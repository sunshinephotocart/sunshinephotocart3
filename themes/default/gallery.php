<?php global $sunshine; load_template(SUNSHINE_PHOTO_CART_PATH.'themes/default/header.php'); ?>
...
if (!sunshine_is_gallery_expired()) {
	if ( post_password_required(SPC()->frontend->current_gallery) ) {
		...
		<div id="sunshine-gallery-list" class="sunshine-clearfix">
			<ul class="sunshine-col-<?php echo esc_attr( SPC()->get_option( 'columns' ) ); ?> sunshine-clearfix">
		...
		<?php }	else {
			...
			if ($images) {
				...
				echo '<ul class="sunshine-col-' . esc_attr( SPC()->get_option( 'columns' ) ) . ' sunshine-clearfix">';
